...
import family.fisa.hangangpaybank.domain.institution.service.BankWalletCommandService;
import family.fisa.hangangpaybank.domain.institution.service.BankWalletQueryService;
...
public class BankWalletController {
    ...
    public ResponseEntity<ApiResponse<BankWalletResponse>> create(
        ...
        // 2. 성공 응답 반환
        return ResponseEntity.status(InstitutionSuccessCode.BANK_WALLET_CREATED.getStatus())
                .body(ApiResponse.onSuccess(InstitutionSuccessCode.BANK_WALLET_CREATED, response));
    }
    ...
    public ResponseEntity<ApiResponse<BankWalletResponse>> get(@PathVariable String walletAddress) {
        ...
        // 2. 성공 응답 반환
        return ResponseEntity.status(InstitutionSuccessCode.BANK_WALLET_DETAIL_OK.getStatus())
                .body(
                        ApiResponse.onSuccess(
                                InstitutionSuccessCode.BANK_WALLET_DETAIL_OK, response));
    }
}
