...
public class UnknownPaymentRecoveryScheduler {
    ...
    public void resolveUnknownPayments() {
        List<Transaction> targets = transactionRepository.findAllUnknownByType(TransactionType.PAYMENT);
    ...
    public void resolveUnknownCancels() {
        ...
        List<Transaction> targets = transactionRepository.findAllUnknownByType(TransactionType.CANCEL);
        ...
        for(Transaction cancelTx : targets) {
            ...
            try {
                ...
                Optional<Transaction> originalPaymentOpt =
                        transactionRepository.findByTransactionUuid(cancelTx.getOriginalTransactionUuid());

                if (originalPaymentOpt.isEmpty()) {
                    // 원본 PAYMENT가 없으면 데이터 정합성 문제 — 재시도해도 해결 안 되므로 ERROR
                    log.error("UNKNOWN 취소 복구 불가: 원본 PAYMENT 없음. cancelUuid={}, originalUuid={}",
                            cancelUuid, cancelTx.getOriginalTransactionUuid());
                    continue;
                }

                // 4. 복구 실행
                transactionCommandService.recoverCancel(merchantPartyId, originalPaymentOpt.get().getId());

            } catch(Exception e) {
                // 5. 개별 실패 — 전체 스케줄러를 중단하지 않고 다음 건 처리
                log.warn("UNKNOWN 취소 복구 실패 (다음 실행에 재시도): cancelUuid={}, reason={}",
                        cancelUuid, e.getMessage());
            }
        }
