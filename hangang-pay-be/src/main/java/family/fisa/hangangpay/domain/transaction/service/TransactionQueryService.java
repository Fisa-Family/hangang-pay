...
public class TransactionQueryService {
    ...
    public CursorPageResponse<PaymentHistoryItem> getUserPaymentHistory(
        ...
        Window<Transaction> window = transactionRepository.findTransactionByPartyId(
                partyId,
        ...
        // 2. 상대방 Party Id 가져오기
        List<Long> toPartyIds = window.getContent().stream().map(t -> t.getToParty().getId()).toList();

        // 3. 가져온 상대방 ID 기반으로 가맹점 조회
        Map<Long, String> merchantNameMap = merchantRepository.findByParty_IdIn(toPartyIds).stream()
                .collect(
                        Collectors.toMap(
                                m -> m.getParty().getId(), Merchant::getMerchantName));

        // 4. PaymentHistoryItem 변환
        Window<PaymentHistoryItem> responseWindow = window.map(
                t -> {
    ...
    public CursorPageResponse<ChargeHistoryItem> getChargeHistories(
        ...
        Window<Transaction> transactions = transactionRepository.findTransactionByPartyId(
                partyId,
                TransactionStatus.SUCCESS,
    ...
    public CursorPageResponse<MerchantPaymentHistoryItem> getMerchantPaymentHistory(
        ...
        Window<Transaction> transactions = transactionRepository.findPaymentTransactionsByMerchantPartyId(
                partyId, TransactionStatus.SUCCESS, position, Limit.of(size));

        List<Long> payerPartyIds = transactions.getContent().stream()
                .map(this::resolvePayerPartyId)
                .distinct()
                .toList();

        Map<Long, String> payerNameMap = userRepository.findByParty_IdIn(payerPartyIds).stream()
                .collect(
                        Collectors.toMap(
                                user -> user.getParty().getId(),
                                user -> maskUsername(user.getUsername())));

        Window<MerchantPaymentHistoryItem> window = transactions.map(
                transaction -> {
                    Long payerPartyId = resolvePayerPartyId(transaction);
                    String payerName = payerNameMap.getOrDefault(payerPartyId, "알 수 없는 사용자");
    ...
    public CursorPageResponse<MerchantSettlementHistoryItem> getMerchantSettlementHistory(
        ...
        Window<MerchantSettlementHistoryItem> window = transactions.map(MerchantSettlementHistoryItem::from);
