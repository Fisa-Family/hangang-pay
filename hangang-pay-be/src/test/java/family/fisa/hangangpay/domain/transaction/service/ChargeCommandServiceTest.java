...
class ChargeCommandServiceTest {
    ...
    void createIntent_위임() {
        ...
        ChargeIntentResponse expected =
                new ChargeIntentResponse(
                        ...
                        "우리은행",
                        LocalDateTime.now());
        when(chargeExecutionWriter.createIntent(eq(PARTY_ID), eq(request), any()))
                .thenReturn(expected);
    ...
    void execute_성공() {
        when(chargeExecutionWriter.prepareProcessing(PARTY_ID, UUID, "1234"))
    ...
    void execute_네트워크오류() {
        ...
        when(chargeExecutionWriter.markUnknown(UUID)).thenReturn(unknown);
