...
contract Settlement is Initializable, UUPSUpgradeable {
  ...
  // 기관 등록 이벤트
  event BankRegistered(uint256 indexed institutionId);

  // 기관 reserve 설정 이벤트
  event ReserveSet(uint256 indexed institutionId, uint256 amount);
  ...
  // 배포 시 CBDC 주소 저장
  function initialize(address _cbdc, address initialOwner) public initializer {
    if (_cbdc == address(0) || initialOwner == address(0)) {
  ...
  // 기관 등록 함수
  function registerBank(uint256 institutionId) external onlyOwner {
    // 기관 ID 검증
    if (institutionId == 0) {
    ...
    emit BankRegistered(institutionId);
  }

  // 기관별 reserve 설정 함수
  // Settlement가 보유한 CBDC 범위 내에서만 가능
  function setReserve(uint256 institutionId, uint256 amount) external onlyOwner {
    // 등록 여부 검증
    ...
    if (totalReserved > ICBDC(cbdc).balanceOf(address(this))) {
    ...
    emit ReserveSet(institutionId, amount);
  }
  ...
  function moveReserve(
    ...
    if (!registeredBank[fromInstitutionId] || !registeredBank[toInstitutionId]) {
      revert BankNotRegistered();
    }

    // reserve 부족 검증
    if (reserveBalance[fromInstitutionId] < amount) {
    ...
    emit ReserveMoved(fromInstitutionId, toInstitutionId, amount);
