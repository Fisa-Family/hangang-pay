...
**새 UI를 구현하기 전에 반드시 `src/components/`를 먼저 확인해야 합니다.**

- 이미 존재하는 공통 컴포넌트(`StatusBadge`, `TxHashCopy`, `BalanceCard` 등)가 있으면 새로 만들지 않고 가져다 씁니다.
- 동일하거나 유사한 UI 패턴을 2곳 이상에서 인라인으로 구현하면 안 됩니다 — 공통 컴포넌트로 추출합니다.
- 공통 컴포넌트를 수정할 때는 모든 사용처에서 깨지지 않는지 확인합니다.
