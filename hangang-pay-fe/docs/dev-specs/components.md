...
| onNavigate | `(path: string) => void` | 탭 이동 요청 |

**UI**

- 화면 하단 고정
- 아이콘과 짧은 라벨 표시
- 사용자 내비게이션은 QR 스캔 탭을 중앙 강조
...
- 서버 처리 대기 또는 실패 상태를 표시한다.

**Props**
| 이름 | 타입 | 설명 |
|---|---|---|
| status | `'loading' \| 'error'` | 표시 상태 |
| loadingText | `string` | 처리중 문구 |
| errorTitle | `string` | 실패 제목 |
| errorMessage | `string?` | 실패 사유 |
| summary | `ReactNode?` | 금액, 가맹점명 등 보조 정보 |
| retryText | `string?` | 재시도 버튼 문구 |
| homeText | `string?` | 홈 이동 버튼 문구 |
| onRetry | `() => void`? | 재시도 |
| onHome | `() => void`? | 홈 이동 |
...
| 상태 | 렌더 |
|---|---|
| loading | 스피너, 처리중 문구, 보조 정보 |
| error | 오류 아이콘, 실패 제목, 실패 사유, 액션 버튼 |
...
- 타입: component
- 사용 화면: 로그인, 회원가입, 비밀번호 찾기, 계좌 입력, 사업자 정보 입력
...
- 타입: component
- 사용 화면: U-PAY-02, U-CHG-01, U-REF-01, M-SET-02
...
- 타입: component
- 사용 화면: 결제 내역, 충전 내역, 환불 내역, 정산 내역, 메뉴 목록
...
- 은행명, 마스킹 계좌번호, 주거래 여부, 계좌 액션을 표시한다.
